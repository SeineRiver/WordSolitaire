...
export default function Home() {
  ...
  return <main className="app-shell"><section ref={canvasRef} className={`game-canvas theme-${theme.id} ${settings.reducedMotion ? 'reduced-motion' : ''}`} aria-label="Solitaire Associations game board">
    ...
    <section className="stock-row" aria-label="Draw pile">
      ...
      {game.waste.at(-1) ? (() => { const card = game.waste.at(-1)!; const visual = cardVisual(card.categoryId, card.label, card.type); return <button ref={(element) => { cardRefs.current[card.id] = element; }} data-card-id={card.id} draggable={settings.interactionMode === 'drag' && !isComplete} onDragStart={() => { if (settings.interactionMode === 'drag') select('waste'); }} className={`waste-card ${card.type} ${visual.kind} ${labelSizeClass(card.label)} ${selectedColumn === 'waste' ? 'selected' : ''}`} onClick={() => select('waste')} disabled={isComplete} aria-label={visual.alt}><span className="card-content">{card.type === 'category' ? renderCategoryVisual(visual, card.label) : renderVisual(visual)}</span></button>; })() : <div className="waste-empty">Open a card</div>}
    ...
    <section className="tableau" aria-label="Tableau cards">{game.tableau.map((column, columnIndex) => {
      ...
      return <div className="word-column" key={columnIndex} onClick={() => column.length === 0 && moveToEmptyColumn(columnIndex)} onDragOver={(event) => { if (settings.interactionMode === 'drag') event.preventDefault(); }} onDrop={(event) => { if (settings.interactionMode === 'drag' && column.length === 0) { event.preventDefault(); moveToEmptyColumn(columnIndex); } }} role={column.length === 0 ? 'button' : undefined} tabIndex={column.length === 0 ? 0 : undefined}>{column.map((card, cardIndex) => {
        ...
        return <button ref={(element) => { cardRefs.current[card.id] = element; }} data-card-id={card.id} draggable={settings.interactionMode === 'drag' && accessible && !isComplete} onDragStart={() => { if (settings.interactionMode === 'drag') select(columnIndex, cardIndex); }} onDragOver={(event) => { if (settings.interactionMode === 'drag') event.preventDefault(); }} onDrop={(event) => { if (settings.interactionMode === 'drag') { event.preventDefault(); select(columnIndex, cardIndex); } }} className={`word-card ${card.type} ${card.faceUp ? 'uncovered' : 'covered'} ${fullyShown ? 'full' : ''} ${lengthClass} ${visual.kind} ${active ? 'selected' : ''}`} disabled={!accessible || isComplete} key={card.id} style={{ top: cardIndex * reveal, zIndex: cardIndex }} onClick={() => select(columnIndex, cardIndex)} aria-label={visual.alt}>{card.faceUp ? <span className="card-content">{card.type === 'category' ? renderCategoryVisual(visual, card.label, !fullyShown) : renderVisual(visual)}</span> : ''}</button>;
      })}</div>;
    })}</section>
  </section></main>;
}
